...
import {AArrowDown, ArrowDown, Github, Linkedin, Mails} from "lucide-react";
    ...
    return (
        <section className="relative h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-white via-gray-50 to-gray-100 pt-20">
            ...
            <div className="absolute inset-1 overflow-hidden">
                <div className="absolute -top-4 -right-40 w-80 h-80 bg-gradient-to-br from-blue-400 to-purple-600 rounded-full opacity-10 blur-3xl animate-pulse"></div>
                ...
                <div className={`transition-all duration-1000 ${ hasAnimated.hero ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} id="hero">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8 animate-fade-in-up delay-300">
                        {heroData.ctaButtons.map((button, index) => (
                            <a href={button.href} key={index} className={`group relative px-5 py-2.5 rounded-lg transition-all duration-300 font-medium text-sm ${button.variant === 'primary' ? 'bg-black text-white shadow-lg hover:shadow-xl hover:bg-gray-800' : 'border-2 border-black text-black hover:bg-black hover:text-white'}`}>
                    ...
                    <div className="flex justify-center space-x-6 mb-8 animate-fade-in-up delay-400">
                        {heroData.socialLinks.map((social, index) => {
                            const IconComponent = social.icon === 'Github' ? Github : social.icon === 'Linkedin' ? Linkedin : Mails;
                            return (
                    ...
                    {/* Stats*/}
                    <div className="grid grid-cols-3 gap-6 max-w-2xl mx-auto mb-8 animate-fade-in-up delay-600">
                    ...
                    <div className="animate-bounce">
                        <button onClick={scrollToAbout} className="grow flex flex-col items-center text-gray-600 hover:text-black transition-colors cursor-pointer">
            ...
            <div className="absolute top-40 right-20 w-6 h-6 bg-purple-400 rounded-full opacity-60 animate-float delay-100"></div>
            <div className="absolute bottom-40 left-20 w-3 h-3 bg-green-400 rounded-full opacity-60 animate-float delay-2000"></div>
            <div className="absolute bottom-20 right-10 w-5 h-5 bg-orange-400 rounded-full opacity-60 animate-float"></div>
        </section>
    );
};
